...
export const hasConfiguredDatabase = Boolean(connectionString);
...
export async function ensureJewelryTable() {
  await sql`
    CREATE TABLE IF NOT EXISTS jewelry_items (
      ...
      setting TEXT,
      tags TEXT,
      metal_images JSONB NOT NULL DEFAULT '{}'::jsonb,
  ...
  await sql`ALTER TABLE jewelry_items ADD COLUMN IF NOT EXISTS subcategory TEXT;`;
  await sql`ALTER TABLE jewelry_items ADD COLUMN IF NOT EXISTS collection TEXT;`;
  await sql`ALTER TABLE jewelry_items ADD COLUMN IF NOT EXISTS stone_type TEXT;`;
  await sql`ALTER TABLE jewelry_items ADD COLUMN IF NOT EXISTS diamond_weight TEXT;`;
  await sql`ALTER TABLE jewelry_items ADD COLUMN IF NOT EXISTS setting TEXT;`;
  await sql`ALTER TABLE jewelry_items ADD COLUMN IF NOT EXISTS tags TEXT;`;
  await sql`ALTER TABLE jewelry_items ADD COLUMN IF NOT EXISTS metal_images JSONB NOT NULL DEFAULT '{}'::jsonb;`;
  await sql`ALTER TABLE jewelry_items ADD COLUMN IF NOT EXISTS gallery_images JSONB NOT NULL DEFAULT '[]'::jsonb;`;
  await sql`ALTER TABLE jewelry_items ADD COLUMN IF NOT EXISTS video_url TEXT;`;
  await sql`ALTER TABLE jewelry_items ADD COLUMN IF NOT EXISTS model_url TEXT;`;
  await sql`ALTER TABLE jewelry_items ADD COLUMN IF NOT EXISTS inventory_status TEXT NOT NULL DEFAULT 'In Stock';`;
  await sql`ALTER TABLE jewelry_items ADD COLUMN IF NOT EXISTS sort_order INTEGER NOT NULL DEFAULT 0;`;
  await sql`ALTER TABLE jewelry_items ADD COLUMN IF NOT EXISTS is_featured BOOLEAN NOT NULL DEFAULT FALSE;`;
  ...
  await sql`CREATE INDEX IF NOT EXISTS idx_jewelry_items_featured ON jewelry_items(is_featured);`;
  await sql`CREATE INDEX IF NOT EXISTS idx_jewelry_items_sort_order ON jewelry_items(sort_order);`;
}
