...
const extractConfirmUrl = (html, id) => {
  ...
  if (confirmMatch?.[1]) {
    return buildDownloadUrl(id, confirmMatch[1]);
  }
...
export default async function handler(req, res) {
  try {
    ...
    if (!response.ok) {
      return res.status(response.status).json({ message: "Drive download failed." });
    }

    const contentType = response.headers.get("content-type") || "";
    if (contentType.includes("text/html")) {
      return res.status(403).json({ message: "Drive file is not publicly accessible." });
    }
    ...
    return res.status(500).json({ message: "Drive proxy failed." });
  }
}
